...
MINIMUM_COST_BY_PART = {
    ...
    "Bonnet": 350000,
    "Trunk lid": 300000,
    "Front fender(L)": 250000,
    "Front fender(R)": 250000,
    "Rear fender(L)": 250000,
    "Rear fender(R)": 250000,
    "Windshield": 400000,
    "Rear windshield": 300000,
    "Head lights(L)": 200000,
    "Head lights(R)": 200000,
    "Side mirror(L)": 100000,
    "Side mirror(R)": 100000,
    "Rocker panel(L)": 200000,
    "Rocker panel(R)": 200000,
    "Front Wheel(L)": 150000,
    "Front Wheel(R)": 150000,
    "Rear Wheel(L)": 150000,
    "Rear Wheel(R)": 150000,
}

DAMAGE_MULTIPLIER = {
    "Scratched": 0.3,    # 스크래치: 30%
    "Separated": 0.7,    # 분리: 70%
    "Crushed": 0.9,      # 찌그러짐: 90%
    "Breakage": 1.0,     # 파손(교체): 100%
}
...
def find_similar_model(user_input):
    ...
    for model in KNOWN_MODELS:
        if user_clean == model.replace(" ", "").lower():
            print(f"🔍 [정확 매칭] '{user_input}' -> '{model}'")
            return model
    ...
    for model in KNOWN_MODELS:
        ...
        if user_clean in model_clean or model_clean in user_clean:
            print(f"🔍 [유사 매칭] '{user_input}' -> '{model}'")
            return model
    
    # 매칭 실패
    print(f"⚠️ 알 수 없는 차종: '{user_input}' -> {DEFAULT_FALLBACK_CAR} 사용")
    return DEFAULT_FALLBACK_CAR
...
def get_cost_prediction(car_model, part_name, damage_type):
    ...
    try:
        ...
        cost = int(max(pred_value, 0))  # 음수 방지
        ...
        if cost < guaranteed_min * 0.5:
            print(f"⚠️ {part_name} 예측값 낮음: {cost:,}원 -> {guaranteed_min:,}원")
            cost = guaranteed_min
        
        # 상한선 (500만원)
        if cost > 5000000:
            print(f"⚠️ {part_name} 예측값 높음: {cost:,}원 -> 상한선 적용")
            cost = 5000000
        ...
        # Fallback
        print(f"⚠️ '{car_model}' 예측 실패. Fallback: {DEFAULT_FALLBACK_CAR}")
...
async def predict(
    ...
    print(f"📝 입력 차종: {car_model} -> 매칭: {matched_car_model}")
    ...
    for part_id in unique_parts:
        ...
        if found_damage:
            cost, used_model = get_cost_prediction(
                matched_car_model, 
                part_name, 
            ...
            print(f"  ✓ {part_name} ({detected_damage_type}): {cost:,}원")

    print(f"💰 총 예상 수리비: {total_estimated_cost:,}원")
    print("="*60)
